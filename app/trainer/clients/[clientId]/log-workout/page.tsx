...
export default function TrainerLogWorkoutPage() {
  ...
  async function openHistory(ex: ExerciseRow) {
    ...
    setHistorySessions(logs.filter(l => byLog[l.id]?.length).map(l => ({ date: l.completed_at, sets: byLog[l.id]! })));
    ...
    try {
      ...
      Object.entries(logged).forEach(([key, val]) => {
        ...
        if (val.reps != null || val.weight != null) {
          rows.push({ client_id: clientId, workout_log_id: wlog.id, exercise_id: exId, set_number: setNum, reps_completed: val.reps, weight_lbs: val.weight, side });
        }
      });
      if (rows.length > 0) {
  ...
  for (const ex of exercises) {
    if (ex.group_id == null) {
      renderItems.push({ kind: "standalone", ex });
    } else if (!seenGroups.has(ex.group_id)) {
      seenGroups.add(ex.group_id);
      renderItems.push({ kind: "group", gid: ex.group_id, items: exercises.filter(e => e.group_id === ex.group_id) });
    }
  }
    ...
    <div style={{ minHeight: "100dvh", background: "#F4F7FA", paddingBottom: 140 }}>
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "16px" }}>
        ...
        <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #E2EAF0", padding: 16, marginBottom: 12 }}>
          ...
          <textarea value={sessionNotes} onChange={e => setSessionNotes(e.target.value)} placeholder="How did the session go?" rows={3}
      ...
      <div style={{ position: "relative", background: "#fff", borderRadius: "20px 20px 0 0", maxHeight: "75dvh", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ overflowY: "auto", padding: "14px 20px 32px", display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {loading ? (
            ...
            const isUnilateral = s.sets.some(r => r.side === "left" || r.side === "right");
            return (
              <div key={i} style={{ background: "#F8FAFB", borderRadius: 12, padding: "12px 14px" }}>
                ...
                {isUnilateral ? (
                  [...new Set(s.sets.map(r => r.set_number))].sort((a, b) => a - b).map(setNum => {
                  ...
                  <div key={j} style={{ display: "flex", alignItems: "center", gap: 12, padding: "5px 0", borderBottom: j < s.sets.length - 1 ? "1px solid #F0F0F0" : "none" }}>
                    ...
                    <div style={{ flex: 1, display: "flex", gap: 16 }}>
                      {r.weight_lbs != null && r.weight_lbs > 0 && <span style={{ fontSize: 14, fontWeight: 700, color: "#0D1827" }}>{r.weight_lbs} <span style={{ fontWeight: 400, color: "#6B7A8D", fontSize: 12 }}>lbs</span></span>}
                      {r.reps_completed != null && <span style={{ fontSize: 14, fontWeight: 700, color: "#0D1827" }}>{r.reps_completed} <span style={{ fontWeight: 400, color: "#6B7A8D", fontSize: 12 }}>reps</span></span>}
                    </div>
                  </div>
                ))}
              </div>
