...
export default function ClientMessagesPage() {
  ...
  async function createConvo() {
    ...
    setGroupName("");
    setCreating(false);
    window.location.href = `/client/messages/${convo.id}`;
  }
    ...
    <div style={{ minHeight: "100dvh", background: "#F4F7FA", paddingBottom: 80 }}>
      <div style={{ background: "#fff", borderBottom: "1px solid #E2EAF0", padding: "16px 20px", position: "sticky", top: 0, zIndex: 10 }}>
        <div style={{ maxWidth: 640, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ fontSize: 20, fontWeight: 800, color: "#1B68B4" }}>Messages</div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 12 }}>
              {peers.map(p => (
                <label key={p.id} style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
                  ...
                  />
                  <div style={{ width: 36, height: 36, borderRadius: "50%", background: avatarColor(p.name), display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 700, fontSize: 13 }}>
                    {initials(p.name)}
                  </div>
                  <span style={{ fontWeight: 600, fontSize: 14, color: "#0D1827" }}>{p.name}</span>
                </label>
              ))}
            </div>
            {selected.size > 1 && (
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "12px 16px" }}>
        {loading ? (
          ...
          <div style={{ background: "#fff", borderRadius: 16, padding: "48px 24px", border: "1px solid #E2EAF0", textAlign: "center" }}>
            ...
            <div style={{ fontSize: 14, color: "#6B7A8D" }}>Tap + New to start a conversation with your trainer or crew.</div>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
            {convos.map(c => {
              ...
              return (
                <Link key={c.id} href={`/client/messages/${c.id}`} style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 16px", background: "#fff", borderRadius: 14, textDecoration: "none", border: c.unread ? "2px solid #1B68B4" : "1px solid #E2EAF0" }}>
                  <div style={{ position: "relative", flexShrink: 0 }}>
                    <div style={{ width: 48, height: 48, borderRadius: "50%", background: av.color, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 800, fontSize: 16 }}>
